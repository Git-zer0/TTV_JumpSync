...
// @version      4.0
// @description  Affiche toujours la fenêtre, même sans Firebase (Mode Local)
// @author       User
...
(function() {
    ...
    // 1. INITIALISATION DES DONNÉES (Firebase ou Local)
    ...
    // 2. MODAL DE CONFIGURATION (S'affiche si rien n'est configuré)
    function showConfigModal() {
        if (document.getElementById('cfg_modal')) return;
        const modal = document.createElement('div');
        modal.id = 'cfg_modal';
        modal.style = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#18181b;padding:20px;border:2px solid #9147ff;border-radius:12px;z-index:1000000005;color:white;font-family:sans-serif;width:300px;display:flex;flex-direction:column;gap:8px;box-shadow:0 0 50px #000;';
        modal.innerHTML = `
            <h3 style="color:#9147ff;margin:0;text-align:center">Configuration</h3>
            ...
            <div style="display:flex;gap:5px;margin-top:10px">
                <button id="f_save" style="flex:1;background:#9147ff;color:white;border:none;padding:10px;cursor:pointer;font-weight:bold">Sync</button>
                <button id="f_local" style="flex:1;background:#444;color:white;border:none;padding:10px;cursor:pointer">Local</button>
            </div>
        `;
    ...
    // 3. INITIALISATION DE L'INTERFACE (VV4)
    function init() {
        if (document.getElementById('twj')) return;
        if (!firebaseConfig && !isLocalMode) { showConfigModal(); return; }
        ...
        d.id = 'twj';
        d.style = `position:fixed;top:100px;left:10px;background:#18181b;padding:12px;border:2px solid #9147ff;border-radius:12px;width:260px;height:auto;color:#fff;font-family:sans-serif;touch-action:none;display:flex;flex-direction:column;gap:10px;z-index:999999999;`;
        ...
        // DRAG & DROP
        let isD = false, oX, oY;
        const start = (e) => { const ev = e.type.includes('touch') ? e.touches[0] : e; isD = true; oX = ev.clientX - d.offsetLeft; oY = ev.clientY - d.offsetTop; };
        const move = (e) => { if (!isD) return; const ev = e.type.includes('touch') ? e.touches[0] : e; d.style.left = (ev.clientX - oX) + 'px'; d.style.top = (ev.clientY - oY) + 'px'; };
        window.addEventListener('mousemove', move); window.addEventListener('touchmove', move, {passive:false});
        window.addEventListener('mouseup', () => isD = false); window.addEventListener('touchend', () => isD = false);
        document.getElementById('h').onmousedown = document.getElementById('h').ontouchstart = start;
        ...
        document.getElementById('g').onclick = () => {
             const parts = I.value.split(':').map(Number);
             if(v()) v().currentTime = (parts[0]*3600) + (parts[1]*60) + parts[2];
        };
