...
(function() {
    ...
    function init() {
        ...
        d.innerHTML = `
            ...
            <div id="g" style="color:#9147ff;font-weight:bold;text-align:center;cursor:pointer;padding:8px;background:#000;border-radius:4px;">
                JUMP
            </div>
        ...
        window.render = () => {
            ...
            sV.slice().reverse().forEach(it => {
                ...
                row.innerHTML = `
                    <span class="jump-it" style="color:#9147ff;cursor:pointer">${it.n}</span>
                    <button class="del" style="background:red;color:white;border:none;font-size:8px">X</button>
                `;
        ...
        document.getElementById('h').onmousedown = (e) => {
            isD = true;
        ...
        // DRAG MOBILE
        document.getElementById('h').addEventListener('touchstart', (e) => {
            const t = e.touches[0];
            isD = true;
    ...
    function waitForTwitchReady() {
        if (!document.body) {
            setTimeout(waitForTwitchReady, 1000);
            return;
        }
    ...
    window.addEventListener('load', () => {
        setTimeout(waitForTwitchReady, 3000);
    });
