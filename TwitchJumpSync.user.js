...
// @version      6.5
// @description  Logique tt6.txt avec invitation config et bascule Sync/Local
// @author       User
...
(function() {
    ...
    // --- 1. GESTION DE L'ÉCRAN DE CONFIGURATION ---
    function showConfigModal() {
        ...
        modal.innerHTML = `
            ...
            <div style="display:flex;gap:5px;margin-top:10px">
                <button id="f_save" style="flex:1;background:#9147ff;color:white;border:none;padding:10px;cursor:pointer;font-weight:bold">Utiliser Firebase</button>
                <button id="f_local" style="flex:1;background:#444;color:white;border:none;padding:10px;cursor:pointer">Mode Local</button>
            </div>
            ${isLocalMode || firebaseConfig ? '<button id="f_close" style="background:none;border:none;color:gray;cursor:pointer;font-size:10px;margin-top:5px">Fermer</button>' : ''}
        `;
        ...
        document.getElementById('f_save').onclick = () => {
            ...
            if(cfg.apiKey) {
                ...
                location.reload();
            } else { alert("API Key requise pour Firebase"); }
        };
        ...
        if (document.getElementById('f_close')) {
            document.getElementById('f_close').onclick = () => modal.remove();
        }
    }

    // Si pas de config et pas de mode local forcé, on affiche l'invitation
    ...
    // --- 3. INITIALISATION DE L'INTERFACE (Basée sur tt6.txt) ---
    function init() {
        ...
        document.getElementById('cfg_gear').onclick = () => {
            if (!isLocalMode) {
                if(confirm("Passer en mode LOCAL ? (Coupe la synchronisation)")) {
                    localStorage.setItem(MODE_KEY, 'true');
                    location.reload();
                }
            } else {
